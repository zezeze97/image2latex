...
@PARSERS.register_module()
class Im2CaptionTextLineStrParser:
    """Parse string of one line in annotation file to dict format.
    ...
    def get_item(self, data_ret, index):
        map_index = index % len(data_ret)
        line_str = data_ret[map_index]
        line_str = line_str.split(self.separator)[:-1]
       
        file_name = line_str[0].split('\t')[0]
        line_str_part = []
        line_str_part.append(file_name)  # file_path
        
        sample = random.randint(0,len(line_str)-1)
        if sample == 0:
            line_str_part.append(line_str[0].split('\t')[1:-1])
        else:
            line_str_part.append(line_str[sample].split('\t')[:-1])
...
@PARSERS.register_module()
class Im2TokenTextLineStrParser:
    """Parse string of one line in annotation file to dict format.
    ...
    def get_item(self, data_ret, index):
        ...
        line_str_part = []
        line_str_part.append(file_name)  # file_path
        line_str_part.append(line_str[1:])
        # line_str_part.append(line_str[0].split('\t')[1:-1])
